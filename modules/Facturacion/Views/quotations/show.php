...
<div class="page-header no-print" style="display:flex;justify-content:space-between;align-items:center;">
    <div>
        ...
        <p>Cotización — <span class="status status-<?= strtolower($doc['status']) ?>"><?= e($doc['status']) ?></span>
        </p>
    </div>
    <div style="display:flex;gap:10px;">
        <?php if ($doc['status'] === 'DRAFT'): ?>
            ...
            <form method="POST" action="<?= url('quotations/cancel/' . $doc['id']) ?>" style="display:inline;">
                <?= csrf_field() ?>
                <button type="submit" class="btn" style="background:var(--danger);color:#fff;"
                    onclick="return confirm('¿Anular esta cotización?')">❌ Anular</button>
            </form>
        <?php endif; ?>
        ...
        <?php endif; ?>
        <a href="<?= url('quotations/print/' . $doc['id']) ?>" target="_blank" class="btn"
...
<div id="printable-document">
    ...
    <!-- Document Title -->
    <h3 class="doc-title">Conduce: <?= e($doc['sequence_code']) ?></h3>
